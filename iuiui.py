...
from tensorflow.keras.optimizers import Adam, RMSprop # Added RMSprop
from tensorflow.keras.losses import BinaryCrossentropy, Huber # Added Huber loss


# פונקציה להבאת נתוני מזון מ-OpenFoodFacts
def fetch_food_data(limit=10000):  # הגדלתי את ה-limit ל-10000
    ...
    try:
        ...
        response.raise_for_status()  # העלה חריגה עבור קודי סטטוס שגויים
        data = response.json()
...
def build_dnn(X_train, y_train, optimizer_name='adam', loss_name='binary_crossentropy'):
    model = Sequential()
    ...
    if loss_name.lower() == 'binary_crossentropy':
    ...
    model.fit(X_train, y_train, epochs=10000, batch_size=32,
              verbose=0)  # Added verbose=0 to suppress training output
    return model
...
if __name__ == "__main__":
    ...
    if X is not None and y is not None:  # בדוק אם preprocess_data החזיר נתונים תקינים
        ...
        for optimizer_name in ['adam', 'rmsprop']:
            for loss_name in ['binary_crossentropy', 'huber']:
                print(f"Training with optimizer: {optimizer_name}, loss: {loss_name}")
                model = build_dnn(X_train, y_train, optimizer_name=optimizer_name, loss_name=loss_name)
