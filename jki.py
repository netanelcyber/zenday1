...
def fetch_ingredient_names(query="avocado", number=100, api_key="1db091aa2f084dfdb05116ef53f9db1e"):
    ...
    if res.status_code == 200:
        ...
        return [item.get("name", "").lower() for item in results if item.get("name")]
    else:
        print("Spoonacular Ingredient API error:", res.status_code, res.text)
        return []
...
def fetch_spoonacular_recipes(query="diabetic", number=100, api_key="1db091aa2f084dfdb05116ef53f9db1e"):
    ...
    if res.status_code == 200:
        ...
        print("Spoonacular Recipe API error:", res.status_code, res.text)
        return []

# === Load recipe data ===
def load_descriptions_spoonacular(api_data, ingredient_keywords, min_score=2):
    descriptions = []
    ...
    for item in api_data or []:
        ...
        if nutrition_score >= min_score and any(kw in full_desc for kw in ingredient_keywords):
...
if __name__ == "__main__":
    ...
    # Step 1: Ingredient scan to build keyword base
    ingredient_keywords = fetch_ingredient_names(query="diabetes", api_key=SPOONACULAR_API_KEY)

    # Step 2: Retrieve matching recipes
    recipe_data = fetch_spoonacular_recipes(query="gestational diabetes", api_key=SPOONACULAR_API_KEY)

    # Step 3: Filter and process
    descriptions = load_descriptions_spoonacular(recipe_data, ingredient_keywords)
