...
def load_descriptions_spoonacular(api_data, min_score=2):
    descriptions = []
    gdm_keywords = [item.get("title", "").lower() for item in api_data if item.get("title")]["low glycemic", "high fiber", "whole grain", "unsweetened", "lean protein", "nonfat", "sugar free", "no sugar added"]
